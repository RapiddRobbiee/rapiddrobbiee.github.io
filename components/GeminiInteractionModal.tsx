...
export const GeminiInteractionModal: React.FC<GeminiInteractionModalProps> = ({
  ai,
  isOpen,
  onClose,
  request,
  ...
  const generatePrompt = useCallback(
    (taskType: GeminiTaskType, data: any, currentUserInput: string): string => {
      let context = '';
      ...
      switch (taskType) {
        case GeminiTaskType.GENERATE_PASSIVE_DESCRIPTION: {
          ...
          const effectsString = passiveSkills
            .map(
              (p) =>
                `- Effect Type ${p.efficacy_type} (Values: ${p.eff_value1}, ${p.eff_value2}, ${p.eff_value3}), Timing: ${p.exec_timing_type}, Target: ${p.target_type}, Condition: ${p.causality_conditions || 'Always'}`
            )
            .join('\n');
        ...
        case GeminiTaskType.GENERATE_LEADER_DESCRIPTION: {
          ...
          const leaderEffectsString = leaderSkills
            .map(
              (ls) =>
                `- Effect Type ${ls.efficacy_type} (Values: ${ls.efficacy_values}), Target: ${ls.target_type} (Sub-target: ${ls.sub_target_type_set_id || 'N/A'}), Condition: ${ls.causality_conditions || 'Always'}`
            )
            .join('\n');
          context += efficacyTypesContext + causalityTypesContext;
  ...
  const requiresUserInput =
    ...
    request.taskType === GeminiTaskType.SUGGEST_EFFICACY_TYPE;
